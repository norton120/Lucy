...
"""The Lucy Schema acts as a canonical representation of the data for all things in Lucy."""
...
class LucySchema(BaseModel):
    """All first-class objects in Lucy are defined as children of LucySchema.
    When different plugins and backends interact they must exchange LucySchema objects (no localized db or platform specific objects).
    """

### GENERATION ###

class ToolCallFunction(LucySchema):
...
class ToolParameter(LucySchema):
...
class Turn(LucySchema):
    """Much like a board game, a 'turn' in Lucy represents one complete pass through the model.
...
class RecallSearchResult(LucySchema):
...
class ArchivalSearchResult(LucySchema):
